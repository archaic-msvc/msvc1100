...
BOOL CJumpList::AddDestination(LPCTSTR lpcszCategoryName, LPCTSTR lpcszDestinationPath)
{
	...
	IShellItem* psi = NULL;
#ifdef UNICODE
	HRESULT hr = SHCreateItemFromParsingName(lpcszDestinationPath, NULL, IID_IShellItem, (LPVOID*)&psi);
#else
	USES_CONVERSION;
	LPWSTR lpDestPath = A2W(lpcszDestinationPath);
	HRESULT hr = SHCreateItemFromParsingName(lpDestPath, NULL, IID_IShellItem, (LPVOID*)&psi);
#endif
