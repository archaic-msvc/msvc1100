...
static HINSTANCE _AfxLoadLangDLL(LPCWSTR pszFormat, LPCWSTR pszPath, LPCWSTR pwzLocaleName)
{
	...
	int nResult = ::GetLocaleInfoEx(pwzLocaleName, LOCALE_SABBREVLANGNAME, szLangCode, 4);
	if (nResult == 0)
	{
		return NULL;
	}
...
HINSTANCE AFXAPI AfxLoadLangResourceDLL(LPCTSTR pszFormat, LPCTSTR pszPath)
{
	...
	bGotPreferredLangs = GetThreadPreferredUILanguages(MUI_LANGUAGE_NAME | MUI_UI_FALLBACK, &nLanguages, wszLanguages, &cchLanguagesBuffer);
...
HRESULT CWinApp::RegisterWithRestartManager(LPCWSTR pwzCommandLineArgs, DWORD dwRestartFlags, APPLICATION_RECOVERY_CALLBACK pRecoveryCallback, LPVOID lpvParam, DWORD dwPingInterval, DWORD dwCallbackFlags)
{
	...
	hr = RegisterApplicationRestart(pwzCommandLineArgs, dwRestartFlags);
	if (hr != S_OK)
	{
		return hr;
	}
	...
	if (pRecoveryCallback != NULL)
	{
		...
		hr = RegisterApplicationRecoveryCallback(pRecoveryCallback, lpvParam, dwPingInterval, dwCallbackFlags);
		if (hr != S_OK)
		{
			return hr;
		}
...
DWORD CWinApp::ApplicationRecoveryCallback(LPVOID /* lpvParam */)
{
	...
	ApplicationRecoveryInProgress(&bRecoveryCanceled);
	...
	ApplicationRecoveryFinished(bRecoverySuccessful);
