...
static void CatchIt(
    ...
    if (continuationAddress != NULL) {
        ...
        // If this is a WinRT exception, make sure to release it before jumping
        if ((pConv != NULL) && CT_ISWINRTHANDLE(*pConv))
        {
...
#pragma prefast(suppress:__WARNING_REDUNDANT_POINTER_TEST __WARNING_REDUNDANT_POINTER_TEST_FAR_EVIDENCE, "Current definition of PER_CODE ensures that pExcept cannot be NULL")
    if ((PER_PTHROW(pExcept) != NULL) && (THROW_UNWINDFUNC(*PER_PTHROW(pExcept)) != NULL)) {
