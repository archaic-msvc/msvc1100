...
#pragma warning(pop)
	...
	VCCORLIB_API void* Heap::Allocate(SizeT size)
	{
		...
		// Extra memory for two pointers for Exception.
		size += 2 * sizeof(void *);

		void* pMem = HeapAllocateHelper(size, 0, 0);
	...
	VCCORLIB_API void* Heap::AlignedAllocate(SizeT size, SizeT alignment)
	{
		...
		// Extra memory for Exception. Binaries built with a compiler that does not have the change to call AlignedAllocateException
		// will call this function to allocate an aligned exception
		size += sizeof(void *);
	...
	VCCORLIB_API void Heap::AlignedFree(void * p, SizeT alignment)
	{
		AlignedFree(p);
	}
	...
	VCCORLIB_API void* Heap::AllocateException(SizeT size)
	{
		...
		// Heap::Allocate will do the additional allocation.
