...
_CRTIMP2_PURE FTYPE __CLRCALL_PURE_OR_CDECL FNAME(Xp_getw)(FTYPE *p, int n)
	{	/* get total value */
	if (n == 0)
		return (FLIT(0.0));
		...
		if (p[3] != FLIT(0.0))
			FSETLSB(p[2]);	/* fold in sticky bit from p[3] */
		if (p01 - p[0] == p[1])
			return (p01 + p[2]);	/* carry is within p[2], add it in */
		else
			return (p[0] + (p[1] + p[2]));	/* fold in p[2] then add it in */
		}
	}
...
_CRTIMP2_PURE FTYPE *__CLRCALL_PURE_OR_CDECL FNAME(Xp_addx)(FTYPE *p, int n, FTYPE *q, int m)
...
_CRTIMP2_PURE FTYPE *__CLRCALL_PURE_OR_CDECL FNAME(Xp_subx)(FTYPE *p, int n, FTYPE *q, int m)
...
_CRTIMP2_PURE FTYPE *__CLRCALL_PURE_OR_CDECL FNAME(Xp_mulx)(FTYPE *p, int n, FTYPE *q, int m,
	FTYPE *ptemp2)
...
_CRTIMP2_PURE FTYPE *__CLRCALL_PURE_OR_CDECL FNAME(Xp_sqrtx)(FTYPE *p, int n, FTYPE *ptemp4)
	...
	if (n == 0)
		...
		if (1 < n)
			x0 += p[1];
		FNAME(Xp_setw)(py, n, FLIT(1.0) / FFUN(sqrt)(x0));	/* py = y */
