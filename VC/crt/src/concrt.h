...
namespace details
{
    ...
    protected:
        ...
        {
            _CONCRT_ASSERT(_M_state != _STATE_CLEAR);
        }
    ...
    private:
        ...
        _CRTIMP void _Invoke();

        volatile long _M_state;
        event *_M_pSyncBlock;
        _CancellationTokenState *_M_pTokenState;
        _CONCRT_BUFFER _M_listEntry[_LISTENTRY_SIZE];
    };
    ...
    public:
        ...
        static _CRTIMP _CancellationTokenState * __cdecl _NewTokenState();

        _CRTIMP ~_CancellationTokenState();
        ...
        _CRTIMP _CancellationTokenRegistration *_RegisterCallback(TaskProc _PCallback, void *_PData, int _InitialRefs = 1);

        _CRTIMP void _RegisterCallback(_CancellationTokenRegistration *_PRegistration);

        _CRTIMP void _DeregisterCallback(_CancellationTokenRegistration *_PRegsitration);

    private:

        // private constructor. Use static function to create an instance.
        _CRTIMP _CancellationTokenState();

        // Ensure that the actual deletion is across the dll boundary
        _CRTIMP virtual void _Destroy();

        // The flag for the token state (whether it is canceled or not)
        volatile long _M_stateFlag;

        // Notification of completion of cancellation of this token.
        event _M_cancelComplete;

        // The protected list of registrations
        _CONCRT_BUFFER _M_registrations[_SAFERWLIST_SIZE];
