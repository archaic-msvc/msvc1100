        ...
        )
{
         ...
         */
        if ( (__flsindex = FlsAlloc(&_freefls)) == FLS_OUT_OF_INDEXES ) {
            _mtterm();
            return FALSE;       /* fail to load DLL */
        }
        ...
        if ( ((ptd = _calloc_crt(1, sizeof(struct _tiddata))) == NULL) ||
             !FlsSetValue(__flsindex, (LPVOID)ptd) )
        {
            _mtterm();
            return FALSE;       /* fail to load DLL */
        }
        ...
        )
{
    ...
    if ( (ptd = FlsGetValue(__flsindex)) == NULL ) {
        /*
        ...
        if ((ptd = _calloc_crt(1, sizeof(struct _tiddata))) != NULL) {
            ...
            if (FlsSetValue(__flsindex, (LPVOID)ptd) ) {
        ...
        )
{
        ...
        if ( __flsindex != FLS_OUT_OF_INDEXES ) {
             ...
             * If FlsGetValue is NULL then ptd could not have been set
             */

            if ( ptd == NULL)
                ptd = FlsGetValue(__flsindex);

            /*
             * Zero out the one pointer to the per-thread data block
             */

            FlsSetValue(__flsindex, (LPVOID)0);
