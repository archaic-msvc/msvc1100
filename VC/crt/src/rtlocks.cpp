...
#define NULL_THREAD_ID -1L
    ...
    public:
        ...
        LockQueueNode(unsigned int timeout = COOPERATIVE_TIMEOUT_INFINITE) 
            : m_pNextNode(NULL), m_ticketState(StateIsBlocked), m_hTimer(NULL), m_trigger(0), m_fTimedNodeInvalid(FALSE)
        {
            ...
            if (timeout != COOPERATIVE_TIMEOUT_INFINITE)
            {
                if (!CreateTimerQueueTimer(&m_hTimer, 
                                            GetSharedTimerQueue(), 
                                            LockQueueNode::DispatchNodeTimeoutTimer, 
                                            this, 
                                            timeout, 
                                            0, 
        ...
        LockQueueNode(Context * pContext, unsigned int ticket, unsigned int timeout = COOPERATIVE_TIMEOUT_INFINITE) 
            : m_pNextNode(NULL), m_pContext(pContext), m_ticketState(ticket), m_hTimer(NULL), m_trigger(0), m_fTimedNodeInvalid(FALSE)
        {
        }
        ...
        bool Unblock()
        {
            if (InterlockedIncrement(&m_trigger) == 1)
            {
    ...
    /// </summary>
    static void CALLBACK DispatchNodeTimeoutTimer(PVOID pContext, BOOLEAN timerOrWaitFired)
    {
        LockQueueNode *pNode = reinterpret_cast<LockQueueNode *> (pContext);
        Context *pUnblockContext = NULL;

        if (InterlockedIncrement(&pNode->m_trigger) == 1)
        {
            pUnblockContext = pNode->m_pContext;
            for(;;)
            {
        ...
        if (InterlockedCompareExchange(&pNode->m_fTimedNodeInvalid, TRUE, FALSE) == TRUE)
        {
            delete pNode;
        }
...
_CRTIMP void critical_section::unlock()
{
    ...
    bool skipUnblock = false;

    if (pNextNode != NULL)
    {
        do
        {
            if (pNextNode->Unblock())
            {
                break;
            }
            else
            {
                LockQueueNode * pPrevious = pNextNode;
                pNextNode = pNextNode->m_pNextNode;
                _M_pHead = pNextNode;
                pCurrentNode = pPrevious;

                // Mark the timed out flag on this node to properly handle cleanup of allocated timer nodes
                // If the flag was true, that means the timer thread has already run and the unlock is responsible
                // for the deletion of the node. Otherwise, we have a proper unlock that occured.
                if (InterlockedCompareExchange(&pPrevious->m_fTimedNodeInvalid, TRUE, FALSE) == TRUE)
                {
                    delete pPrevious;
                }
            }
...
bool critical_section::_Acquire_lock(void * _PLockingNode, bool _FHasExternalNode)
{
    ...
    return !pNewNode->m_fTimedNodeInvalid;
}
