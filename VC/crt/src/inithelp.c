...
error:
        ...
        } else if (lc_type == LC_WSTR_TYPE)
        {
            ...
            *wstraddress = NULL;
            ...
            if ((buffersize = GetLocaleInfoEx(localename, fieldtype, NULL, 0)) == 0)
                goto werror;

            if ((*wstraddress = (wchar_t *) _calloc_crt(buffersize, sizeof(wchar_t))) == NULL)
                goto werror;

            if (GetLocaleInfoEx(localename, fieldtype, *wstraddress, buffersize) == 0)
                goto werror;
...
werror:
        ...
        } else if (lc_type == LC_INT_TYPE)
        {
            ...
            if (GetLocaleInfoEx (localename, fieldtype | LOCALE_RETURN_NUMBER, (LPWSTR)&dw, sizeof(dw) / sizeof(WCHAR)) == 0)
                return -1;
