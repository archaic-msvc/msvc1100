...
// All CRT public head files are required to be protected from macro new
...
/// <summary>
///     This class describes an exception thrown when there is an unobserved exception stored in the internal state associated
///     with a task, and all <c>task</c> objects associated with the internal task have been destructed.
/// </summary>
/// <remarks>
///     A PPL task executes asynchronously and you are not required to retrieve the result of the task using either the
///     <see cref="task::get Method">task::get</see> or the <see cref="task::wait Method">task::wait</see> methods.
///     <para>Exceptions thrown by the task body are captured on the thread on which they are thrown, and stored in the
///     internal state associated with the task, to be thrown out of a subsequent call to <c>get</c> or <c>wait</c>.</para>
///     <para>However if you never call the <c>get()</c> or <c>wait()</c> methods on either the task or any of the tasks in its
///     continuation tree that take an input of type <c>task::result_type</c>, the exception is considered to be unhandled, and is
///     surfaced when all copies of the <c>task</c> objects that have access to that stored exception go out of scope.</para>
///     <para>The purpose of the <c>unobserved_task_exception</c> class is to alert you to the fact that an unhandled exception was
///     encountered, and to allow you to diagnose problems with your program that would be hard to find if the Runtime silently
///     intercepted and supressed the exception.</para>
///     <para>If you encounter an <c>unobserved_task_exception</c> while debugging your program, turn on first chance exceptions in the
///     debugger so you can catch the unhandled exception sooner. Once the source of the <c>unobserved_task_exception</c> you should fix
///     your program so the exception is either not thrown, or caught and handled appropriately. Under no circumstances should you try
///     to catch an instance of <c>unobserved_task_exception</c> in a catch clause and proceed with execution.</para>
/// </remarks>
/// <seealso cref="task Class"/>
/**/
...
public:
    /// <summary>
    ///     Constructs a <c>unobserved_task_exception</c> object.
    /// </summary>
    /// <param name="_Message">
    ///     A descriptive message of the error.
    /// </param>
    /// <param name="_Exception">
    ///     An <c>std::exception_ptr</c> object representing the exception that was encountered by the Runtime.
    /// </param>
    /**/
    explicit unobserved_task_exception(_In_z_ const char* _Message, const std::exception_ptr& _Exception) throw()
        : std::exception(_Message), _M_stdException(_Exception)
#if defined (__cplusplus_winrt)
    ...
    /// </param>
    /**/
    explicit unobserved_task_exception(const std::exception_ptr& _Exception) throw()
        : std::exception(), _M_stdException(_Exception)
#if defined (__cplusplus_winrt)
        , _M_winRTException(nullptr)
#endif  /* defined (__cplusplus_winrt) */
    {
    ...
    /// </param>
    /// <param name="_Exception">
    ///     A reference to a <c>Platform::Exception</c> object representing the exception that was encountered by the Runtime.
    /// </param>
    /**/
    explicit unobserved_task_exception::unobserved_task_exception(const char* _Message, Platform::Exception^ _Exception) throw()
        : std::exception(_Message), _M_winRTException(_Exception)
    {
    }
    ...
    /// </param>
    /**/
    explicit unobserved_task_exception::unobserved_task_exception(Platform::Exception^ _Exception) throw()
        : std::exception(), _M_winRTException(_Exception)
    {
    }
#endif  /* defined (__cplusplus_winrt) */

    /// <summary>
    ///     A method that throws the exception that this object wraps.
    /// </summary>
    /**/
...
namespace details
{
    ...
    template <typename _Function> auto _IsCallable(_Function _Func, int) -> decltype(_Func(), std::true_type()) { return std::true_type(); }
    ...
    /// </param>
    static void _ScheduleFuncWithAutoInline(const std::function<void ()> & _Func, _TaskInliningMode _InliningMode)
    {
        _StackGuard _Guard;
        ...
        {
            _CurrentScheduler::_ScheduleTask(reinterpret_cast<TaskProc>(&_TaskProcThunk::_Bridge), new _TaskProcThunk(_Func));
        }
    }
    ...
    struct _MarshalHelper
    {
        static _Type^ _Perform(_Type^ _ObjInCtx, const _ContextCallback& _Ctx)
        {
            ...
            IStream * _PStream;
            _Ctx._CallInContext([&](){
                // It isn't safe to simply reinterpret_cast a hat type to IUnknown* because some hat types do not have a real vtable ptr.
                // Instead, we need to first upcast it to Object^ to make it "grow" the vtable ptr.
                Platform::Object^ _PObj = safe_cast<Platform::Object^>(_ObjInCtx);
                IUnknown * _PUnk = reinterpret_cast<IUnknown*>(_PObj);
                _Hr = CoMarshalInterThreadInterfaceInStream(__uuidof(_Type^), _PUnk, &_PStream);
            });
            ...
            //
            _Hr = CoGetInterfaceAndReleaseStream(_PStream, __uuidof(_Type^), reinterpret_cast<void**>(&_Proxy));
            if (FAILED(_Hr))
            {
                throw Platform::Exception::CreateException(_Hr);
            }
    ...
    // The exception is 'observed' if the user invokes get()/wait() or cancels any of the tasks that are sharing this exception holder. If the exception
    // is not observed by the time the internal object owned by the shared pointer destructs, an unobserved_task_exception is thrown by the Runtime.
    struct _ExceptionHolder
    {
        explicit _ExceptionHolder(const std::exception_ptr& _E) : _M_exceptionObserved(0), _M_storedException(_E)
        {
        }

#if defined (__cplusplus_winrt)
        explicit _ExceptionHolder(Platform::Exception^ _E) : _M_exceptionObserved(0),  _M_storedException(_E)
        {
        }
#endif  /* defined (__cplusplus_winrt) */
        ...
        ~_ExceptionHolder()
        {
            ...
            {
                // Throwing an exception from a destructor will terminate the process if the destructor is executing as part of stack
                // unwinding due to an existing exception. In general, this should be fine here, since we're throwing this exception in
                // order to terminate the process.
                throw _M_storedException;
            }
        }
        ...
        void _RethrowUserException()
        {
            ...
            _M_storedException.throw_stored_exception();
        }

        long volatile _M_exceptionObserved;
        // Store the unobserved task exception for convenience and code reuse.
        unobserved_task_exception _M_storedException;
    };
    ...
    internal:
        ...
        _AsyncOperationType _M_asyncInfo;
    ...
    public:
        virtual void Cancel() { _M_asyncInfo->Cancel(); }
        virtual void Close() { _M_asyncInfo->Close(); }

        virtual property Windows::Foundation::HResult ErrorCode
        {
            Windows::Foundation::HResult get()
            {
                return _M_asyncInfo->ErrorCode;
            }
        }
        ...
        virtual property Windows::Foundation::AsyncOperationCompletedHandler<_Result>^ Completed
        {
            ...
            void set(Windows::Foundation::AsyncOperationCompletedHandler<_Result>^ value)
            {
                ...
                _M_asyncInfo->Completed = ref new _CompletionHandlerType([&](_AsyncOperationType, Windows::Foundation::AsyncStatus status) {
                    ...
                    // _AsyncInit.
                    _M_asyncInfo = _ResultContext<_AsyncOperationType>::_GetValue(_M_asyncInfo, _M_asyncInfoContext, false);
    ...
    internal:
        _IAsyncOperationWithProgressToAsyncOperationConverter(Windows::Foundation::IAsyncOperationWithProgress<_Result,_Progress>^ _Operation) :
            _AsyncInfoImpl<Windows::Foundation::IAsyncOperationWithProgress<_Result,_Progress>^,
                          ...
                          _Result>(_Operation) {}
    public:
        virtual _Result GetResults() override { return _M_asyncInfo->GetResults(); }
    };
...
///     It is only useful to use this class from a Metro style app. For non-Metro style apps, the task continuation's
...
public:
    ...
    ///     task continuations will execute. However, in a Metro style app, the default continuation context for a continuation on an
    ...
    ///     <para>This method is only available to Metro style apps.</para>
    /// </remarks>
    /**/
...
namespace details
{
    ...
    struct _Task_ptr
    {
        ...
        static _Type _Make(_CancellationTokenState * _Ct) { return std::make_shared<_Task_impl<_ReturnType>>(_Ct); }
    };
    ...
    struct _ContinuationTaskHandleBase : _UnrealizedChore
    {
        ...
        _ContinuationTaskHandleBase() :
            _M_next(nullptr), _M_isTaskBasedContinuation(false), _M_continuationContext(task_continuation_context::use_default()), _M_inliningMode(_NoInline)
        {
        }
    ...
    ///     For initial tasks, _PPLTaskHandlewill be derived from _UnrealizedChore, and for continuation tasks, it will be derived from
    ...
    struct _PPLTaskHandle : _BaseTaskHandle
    {
        ...
        {
            m_pFunction = reinterpret_cast <TaskProc> (&_UnrealizedChore::_InvokeBridge<_PPLTaskHandle>);
    ...
    struct _Task_impl_base
    {
        ...
        _Task_impl_base(_CancellationTokenState * _PTokenState) : _M_TaskState(_Created),
        ...
        bool _CancelWithException(Platform::Exception^ _Exception)
        {
            ...
            return _CancelAndRunContinuations(true, true, false, std::make_shared<_ExceptionHolder>(_Exception));
        }
#endif  /* defined (__cplusplus_winrt) */
        ...
        {
            _CONCRT_ASSERT(details::_CancellationTokenState::_IsValid(_M_pTokenState));
            _M_pRegistration = _M_pTokenState->_RegisterCallback(reinterpret_cast<TaskProc>(&_CancelViaToken), (_Task_impl_base *)this);
        }
        ...
        void _ScheduleTask(_UnrealizedChore * _PTaskHandle, _TaskInliningMode _InliningMode)
        {
            ...
            _M_pTaskCollection = _AsyncTaskCollection::_NewCollection(_CancellationTokenState::_None());
           ...
           // Ensure that the continuation runs in proper context (this might be on a Concurrency Runtime thread or in a different Windows Runtime apartment)
            if (_PTaskHandle->_M_continuationContext._HasCapturedContext())
            {
                ...
                _ScheduleFuncWithAutoInline([_PTaskHandle]() {
                    ...
                    {
                        _TaskImplPtr->_ScheduleTask(_PTaskHandle, _ForceInline);
                    }
                    else
                    {
                        //
                               ...
                               Windows::Foundation::IAsyncOperation<typename details::_ValueTypeOrRefType<_ReturnType>::_Value>^ _AsyncOp)
        {
              ...
              [_OuterTask](Windows::Foundation::IAsyncOperation<typename details::_ValueTypeOrRefType<_ReturnType>::_Value>^ _Operation, Windows::Foundation::AsyncStatus _Status) mutable
            {
                ...
                {
                    _OuterTask->_CancelWithException(Platform::Exception::CreateException(static_cast<int>(_Operation->ErrorCode.Value)));
                }
                else
                {
                    _CONCRT_ASSERT(_Status == Windows::Foundation::AsyncStatus::Completed);
        ...
        // An exception thrown by the task body is captured in an exception holder and it is shared with all its value based continuations.
        // The exception is 'observed' if the user invokes get()/wait() or schedules a task-based continuation on any of the tasks that are sharing this
        // exception holder. If the exception is not observed by the time the internal object owned by the shared pointer destructs,
        // an unobserved_task_exception is thrown by the runtime.
...
#endif  /* defined (__cplusplus_winrt) */
        _Task_impl(_CancellationTokenState * _Ct): _Task_impl_base(_Ct)
        {
#if defined (__cplusplus_winrt)
        ...
        virtual bool _CancelAndRunContinuations(bool _SynchronousCancel, bool _UserException, bool _PropagatedFromAncestor, const std::shared_ptr<_ExceptionHolder> & _ExceptionHolder)
        {
            ...
            if (_RunContinuations)
            {
                ...
                if (_M_Continuations)
                {
                    ...
                    _ScheduleFuncWithAutoInline([=](){ _RunTaskContinuations(); }, _DefaultAutoInline);
                }
            }
            return true;
        }
...
public:
    ...
    bool set(_ResultType _Result) const // 'const' (even though it's not deep) allows to safely pass events by value into lambdas
    {
        ...
        bool _RunContinuations = false;
        {
            critical_section::scoped_lock _LockHolder(_M_Impl->_M_taskListCritSec);
    ...
    bool _Cancel(_ExHolderType _ExHolder) const
    {
        bool _Canceled;
    ...
    /// </summary>
    template<typename _ExHolderType>
    bool _StoreException(_ExHolderType _ExHolder) const
    {
        critical_section::scoped_lock _LockHolder(_M_Impl->_M_taskListCritSec);
        if (!_IsTriggered() && !_M_Impl->_HasUserException())
        {
            _M_Impl->_M_exceptionHolder = _ToExceptionHolder(_ExHolder);
            return true;
        }
        return false;
    }
...
private:
    ...
    static std::shared_ptr<details::_ExceptionHolder> _ToExceptionHolder(const std::shared_ptr<details::_ExceptionHolder>& _ExHolder)
    {
        return _ExHolder;
    }

    static std::shared_ptr<details::_ExceptionHolder> _ToExceptionHolder(std::exception_ptr _ExceptionPtr)
    {
        return std::make_shared<details::_ExceptionHolder>(_ExceptionPtr);
    }
...
public:
    ...
    bool set_exception(std::exception_ptr _ExceptionPtr) const // 'const' (even though it's not deep) allows to safely pass events by value into lambdas
    {
        return _M_unitEvent.set_exception(_ExceptionPtr);
    }
...
public:
    ...
    ///     object, or a Windows::Foundation::IAsyncInfo if you are using tasks in your Metro style app. The lambda or function
    ///     object should be a type equivalent to <c>std::function&lt;X(void)&gt;</c>, where X can be a variable of type <c>result_type</c>,
    ///     <c>task&lt;result_type&gt;</c>, or a Windows::Foundation::IAsyncInfo in Metro style apps.
    /// </param>
    /// <remarks>
    ///     The default constructor for a <c>task</c> is only present in order to allow tasks to be used within containers.
    ///     A default constructed task cannot be used until you assign a valid task to it. Methods such as <c>get</c>, <c>wait</c> or <c>then</c>
    ...
    explicit task(_Ty _Param)
    {
        ...
        _CreateImpl(cancellation_token::none()._GetImplValue());
    ...
    /**/
    template<typename _Ty>
    explicit task(_Ty _Param, cancellation_token _Token)
    {
        details::_ValidateTaskConstructorArgs<_ReturnType,_Ty>(_Param);
    ...
    {
        return _ThenImpl<_ReturnType, _Function>(_Func, nullptr, task_continuation_context::use_default());
    }
    ...
    /**/
    template<typename _Function>
    auto then(const _Function& _Func, cancellation_token _CancellationToken) const -> typename details::_ContinuationTypeTraits<_Function, _ReturnType>::_TaskOfType
    {
        return _ThenImpl<_ReturnType, _Function>(_Func, _CancellationToken._GetImplValue(), task_continuation_context::use_default());
    }
    ...
    ///     A variable that specifies where the continuation should execute. This variable is only useful when used in a Metro
    ...
    {
        return _ThenImpl<_ReturnType, _Function>(_Func, nullptr, _ContinuationContext);
    }
#endif  /* defined (__cplusplus_winrt) */
    ...
    {
        return _ThenImpl<_ReturnType, _Function>(_Func, _CancellationToken._GetImplValue(), _ContinuationContext);
    }
    ...
    /// </summary>
    void _CreateImpl(details::_CancellationTokenState * _Ct)
    {
    ...
    /// </summary>
    template<typename _Function>
    auto _Then(const _Function& _Func, details::_CancellationTokenState *_PTokenState, bool _Aggregating,
        details::_TaskInliningMode _InliningMode = details::_ForceInline) const -> typename details::_ContinuationTypeTraits<_Function, _ReturnType>::_TaskOfType
    {
...
private:
        ...
        details::_PPLTaskHandle<_ReturnType, _InitialTaskHandle<_InternalReturnType, _Function, _TypeSelection>, details::_UnrealizedChore>
    {
        ...
        {
            details::_Task_impl_base::_AsyncInit<_ReturnType, _InternalReturnType>(_M_pTask, ref new details::_IAsyncActionToAsyncOperationConverter(_M_function()));
        }
        ...
        void _Init(details::_TypeSelectorAsyncOperationWithProgress) const
        {
            ...
            details::_Task_impl_base::_AsyncInit<_ReturnType, _InternalReturnType>(_M_pTask,
                    ref new details::_IAsyncOperationWithProgressToAsyncOperationConverter<_InternalReturnType,_ProgressType>(_M_function()));
        }
        ...
        void _Init(details::_TypeSelectorAsyncActionWithProgress) const
        {
            ...
            details::_Task_impl_base::_AsyncInit<_ReturnType, _InternalReturnType>(_M_pTask,
                ref new details::_IAsyncActionWithProgressToAsyncOperationConverter<_ProgressType>(_M_function()));
        }
#endif  /* defined (__cplusplus_winrt) */
    };
        ...
        _ContinuationTaskHandle<_InternalReturnType, _ContinuationReturnType, _Function, _IsTaskBased, _TypeSelection>, details::_ContinuationTaskHandleBase>
    {
        ...
        void _Continue(std::true_type, details::_TypeSelectorAsyncAction) const
        {
            ...
            details::_Task_impl_base::_AsyncInit<_NormalizedContinuationReturnType, _ContinuationReturnType>(_M_pTask,
                ref new details::_IAsyncActionToAsyncOperationConverter(_M_function(std::move(_ResultTask))));
        }
        ...
        void _Continue(std::true_type, details::_TypeSelectorAsyncOperationWithProgress) const
        {
            ...
            details::_Task_impl_base::_AsyncInit<_NormalizedContinuationReturnType, _ContinuationReturnType>(_M_pTask,
                    ref new details::_IAsyncOperationWithProgressToAsyncOperationConverter<_ContinuationReturnType, _ProgressType>(_M_function(std::move(_ResultTask))));
        }
        ...
        void _Continue(std::true_type, details::_TypeSelectorAsyncActionWithProgress) const
        {
            ...
            details::_Task_impl_base::_AsyncInit<_NormalizedContinuationReturnType, _ContinuationReturnType>(_M_pTask,
                    ref new details::_IAsyncActionWithProgressToAsyncOperationConverter<_ProgressType>(_M_function(std::move(_ResultTask))));
        }
#endif  /* defined (__cplusplus_winrt) */
    };
    ...
    void _TaskInitWithFunctor(const _Function& _Func)
    {
        ...
        _M_Impl->_ScheduleTask(new _InitialTaskHandle<_InternalReturnType, _Function, _Async_type_traits::_AsyncKind>(_GetImpl(), _Func), details::_NoInline);
    }
    ...
    /// </summary>
    template<typename _InternalReturnType, typename _Function>
    auto _ThenImpl(const _Function& _Func, details::_CancellationTokenState *_PTokenState, const task_continuation_context& _ContinuationContext,
        bool _Aggregating = false, details::_TaskInliningMode _InliningMode = details::_NoInline) const -> typename details::_ContinuationTypeTraits<_Function, _InternalReturnType>::_TaskOfType
    {
        if (_M_Impl == nullptr)
        {
        ...
        typedef details::_TaskTypeTraits<_Function_type_traits::_FuncRetType> _Async_type_traits;
        typedef _Async_type_traits::_TaskRetType _TaskType;
        ...
        _GetImpl()->_ScheduleContinuation(new _ContinuationTaskHandle<_InternalReturnType, _TaskType, _Function, _Function_type_traits::_Takes_task, _Async_type_traits::_AsyncKind>(
...
public:
    ...
    explicit task(_Ty _Param)
    {
        ...
        _M_unitTask._CreateImpl(cancellation_token::none()._GetImplValue());
    ...
    /**/
    template<typename _Ty>
    explicit task(_Ty _Param, cancellation_token _CancellationToken)
    {
        details::_ValidateTaskConstructorArgs<void,_Ty>(_Param);
    ...
    {
        return _M_unitTask._ThenImpl<void, _Function>(_Func, nullptr, task_continuation_context::use_default());
    }
    ...
    /**/
    template<typename _Function>
    auto then(const _Function& _Func, cancellation_token _CancellationToken) const -> typename details::_ContinuationTypeTraits<_Function, void>::_TaskOfType
    {
        return _M_unitTask._ThenImpl<void, _Function>(_Func, _CancellationToken._GetImplValue(), task_continuation_context::use_default());
    }
    ...
    {
        return _M_unitTask._ThenImpl<void, _Function>(_Func, nullptr, _ContinuationContext);
    }
#endif  /* defined (__cplusplus_winrt) */
    ...
    {
        return _M_unitTask._ThenImpl<void, _Function>(_Func, _CancellationToken._GetImplValue(), _ContinuationContext);
    }
    ...
    auto _Then(const _Function& _Func, details::_CancellationTokenState *_PTokenState,
        bool _Aggregating, details::_TaskInliningMode _InliningMode = details::_ForceInline) const -> typename details::_ContinuationTypeTraits<_Function, void>::_TaskOfType
    {
...
///     object, a different <c>task</c> object, or a Windows::Foundation::IAsyncInfo interface if you are using tasks in your Metro style app.
/// </param>
/// <returns>
...
///     <para>In a Metro style app, if <paramref name="_Param"/> is of type Windows::Foundation::IAsyncOperation&ltT&gt^ or
...
auto create_task(_Ty _Param) -> task<typename details::_TaskTypeFromParam<_Ty>::_Type>
{
    ...
    return task<typename details::_TaskTypeFromParam<_Ty>::_Type>(_Param);
}
...
/**/
template<typename _Ty>
auto create_task(_Ty _Param, cancellation_token _Token) -> task<typename details::_TaskTypeFromParam<_Ty>::_Type>
{
    static_assert(!std::is_same<typename details::_TaskTypeFromParam<_Ty>::_Type,details::_BadArgType>::value,
#if defined (__cplusplus_winrt)
            "incorrect argument for create_task; can be a callable object, an asynchronous operation, or a task_completion_event"
    ...
    return task<typename details::_TaskTypeFromParam<_Ty>::_Type>(_Param, _Token);
}
...
/// </typeparam>
/// <remarks>
///     This type is only available to Metro style apps.
/// </remarks>
...
namespace details
{
    ...
    enum _AsyncStatusInternal
    {
        ...
        _AsyncStarted = Windows::Foundation::AsyncStatus::Started, // 0
        _AsyncCompleted = Windows::Foundation::AsyncStatus::Completed, // 1
        _AsyncCanceled = Windows::Foundation::AsyncStatus::Canceled, // 2
        _AsyncError = Windows::Foundation::AsyncStatus::Error, // 3
    ...
    {
        template<typename _Function>
        static task<_ReturnType> _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts)
        {
            return task<_ReturnType>(_Func(), _Cts.get_token());
        }

        template<typename _Function>
        static task<_ReturnType> _GenerateTask_1C(const _Function& _Func, cancellation_token_source _Cts)
        {
            return task<_ReturnType>(_Func(_Cts.get_token()), _Cts.get_token());
        }

        template<typename _Function, typename _ProgressObject>
        static task<_ReturnType> _GenerateTask_1P(const _Function& _Func, const _ProgressObject& _Progress, cancellation_token_source _Cts)
        {
            return task<_ReturnType>(_Func(_Progress), _Cts.get_token());
        }

        template<typename _Function, typename _ProgressObject>
        static task<_ReturnType> _GenerateTask_2PC(const _Function& _Func, const _ProgressObject& _Progress, cancellation_token_source _Cts)
        {
            return task<_ReturnType>(_Func(_Progress, _Cts.get_token()), _Cts.get_token());
        }
    };
    ...
    {
        template<typename _Function>
        static task<void> _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts)
        {
            return task<void>(_Func(), _Cts.get_token());
        }

        template<typename _Function>
        static task<void> _GenerateTask_1C(const _Function& _Func, cancellation_token_source _Cts)
        {
            return task<void>(_Func(_Cts.get_token()), _Cts.get_token());
        }

        template<typename _Function, typename _ProgressObject>
        static task<void> _GenerateTask_1P(const _Function& _Func, const _ProgressObject& _Progress, cancellation_token_source _Cts)
        {
            return task<void>(_Func(_Progress), _Cts.get_token());
        }

        template<typename _Function, typename _ProgressObject>
        static task<void> _GenerateTask_2PC(const _Function& _Func, const _ProgressObject& _Progress, cancellation_token_source _Cts)
        {
    ...
    struct _TaskGenerator<_Generator, false, false>
    {
        ...
        static auto _GenerateTask(const _Function& _Func, _ClassPtr _Ptr, cancellation_token_source _Cts)
    ...
    struct _AsyncAttributes<_Function, _ProgressType, _ReturnType, _TaskTraits, _TakesToken, true>
    {
        ...
        typedef typename _SelectorTaskGenerator<typename _TaskTraits::_AsyncKind, _ReturnType> _SelectorTaskGenerator;
        ...
        static task<_ReturnType> _Generate_Task(const _Function& _Func, _ClassPtr _Ptr, cancellation_token_source _Cts)
        {
    ...
    public:
        ...
        property typename _Attributes::_ProgressDelegateType^ Progress
        {
...
#pragma warning(pop)
    ...
    private:
        ...
        typename _Attributes::_CompletionDelegateType^  _M_completeDelegate;
    ...
    protected:
        ...
        virtual void _OnStart() override
        {
            ...
            //
            _M_task = _Attributes::_Generate_Task(_M_func, this, _M_cts);
            _Base::_OnStart();
        }
...
///     <para>This function is only available to Metro style apps.</para>
/// </remarks>
/// <seealso cref="task Class"/>
...
auto create_async(const _Function& _Func) -> decltype(ref new details::_AsyncTaskGeneratorThunk<_Function>(_Func))
{
    ...
    return ref new details::_AsyncTaskGeneratorThunk<_Function>(_Func);
}
...
namespace details
{
    ...
    struct _RunAllParam
    {
        ...
        _Type                                   _M_mergeVal;
        volatile size_t                         _M_completeCount;
        size_t                                  _M_numTasks;
    };
    ...
    struct _WhenAllImpl
    {
        static task<std::vector<_ElementType>> _Perform(_CancellationTokenState *_PTokenState, _Iterator _Begin, _Iterator _End)
        {
    ...
    {
        static task<void> _Perform(_CancellationTokenState *_PTokenState, _Iterator _Begin, _Iterator _End)
        {
...
{
    typedef std::iterator_traits<_Iterator>::value_type::result_type _ElementType;
...
namespace details
{
    ...
    struct _WhenAnyImpl
    {
        static task<std::pair<_ElementType, size_t>> _Perform(_CancellationTokenState *_PTokenState, _Iterator _Begin, _Iterator _End)
        {
            if( _Begin == _End )
            {
            ...
            task<std::pair<std::pair<_ElementType, size_t>, _CancellationTokenState *>> _Any_tasks_completed(_PParam->_M_Completed, _PParam->_M_cancellationSource.get_token());
            ...
            for (auto _PTask = _Begin; _PTask != _End; ++_PTask)
            {
                ...
                _PTask->_Then([_PParam, index](task<_ElementType> _ResultTask) {
    ...
    {
        static task<size_t> _Perform(_CancellationTokenState *_PTokenState, _Iterator _Begin, _Iterator _End)
        {
            if( _Begin == _End )
            {
                throw invalid_operation("when_any(begin, end) cannot be called on an empty container.");
            ...
            task<std::pair<size_t, _CancellationTokenState *>> _Any_tasks_completed(_PParam->_M_Completed, _PParam->_M_cancellationSource.get_token());
            ...
            for (auto _PTask = _Begin; _PTask != _End; ++_PTask)
            {
                ...
                _PTask->_Then([_PParam, index](task<void> _ResultTask) {
                    auto _Func = [&_ResultTask, _PParam, index]() {
...
{
    auto _PParam = new details::_RunAnyParam<std::pair<_ReturnType, details::_CancellationTokenState *>>();

    task<std::pair<_ReturnType, details::_CancellationTokenState *>> _Any_tasks_completed(_PParam->_M_Completed, _PParam->_M_cancellationSource.get_token());
    ...
    auto _ReturnTask = _Any_tasks_completed._Then([=](std::pair<_ReturnType, details::_CancellationTokenState *> _Ret) -> _ReturnType {
    ...
    _Lhs._Then(_Continuation, details::_CancellationTokenState::_None(), false);
    _Rhs._Then(_Continuation, details::_CancellationTokenState::_None(), false);
