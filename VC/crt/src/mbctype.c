...
const wchar_t *__mblocalename = NULL; /* __initialmbcinfo.mblocalename; */
...
static void setSBUpLow (pthreadmbcinfo ptmbci)
{
    ...
    if (GetCPInfo(ptmbci->mbcodepage, &cpInfo) != 0)
    {
        ...
        __crtLCMapStringA(NULL, ptmbci->mblocalename, LCMAP_LOWERCASE, (LPCSTR)sbVector, 256,
                                    (LPSTR)lowVector, 256, ptmbci->mbcodepage, FALSE);

        //  get upper case mappings for character vector

        __crtLCMapStringA(NULL, ptmbci->mblocalename, LCMAP_UPPERCASE, (LPCSTR)sbVector, 256,
            ...
            if ( (retcode = _setmbcp_nolock(codepage, ptmbci)) == 0)
            {
                ...
                if (!(ptd->_ownlocale & _PER_THREAD_LOCALE_BIT) &&
                    ...
                    __try
                    {
                        ...
                        __mblocalename = ptmbci->mblocalename;
                        for ( i = 0 ; i < 5 ; i++ )
