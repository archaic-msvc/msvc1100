...
// ISSUE-2011/04/10-MarcusMa - Exchange does not have SAL v2 yet, so we need
// to redefine the v2 markup to v1. This is not a comprehensive list but rather all the
// hits up to this point. See http://windows/toolbox/Analysis/SAL/SAL 1 to SAL 2 Conversion Table.xlsx
#ifdef SAL_VERSION_SAL2
// Nothing as Windows 8 is using SAL v2.
#else // ESENT
#define _In_reads_(x)						__in_ecount( x )
#define _In_reads_opt_(x)					__in_ecount_opt( x )
#define _In_reads_bytes_(x)					__in_bcount( x )
#define _In_reads_bytes_opt_(x)				__in_bcount_opt( x )
#define _Inout_updates_bytes_(x)			__inout_bcount( x )
#define _Inout_updates_opt_(x)				__inout_ecount_opt( x )
#define _Out_writes_(x)						__out_ecount( x )
#define _Out_writes_to_(x, y)				__out_ecount_part( x, y )
#define _Out_writes_to_opt_(x, y)			__out_xcount_part_opt( x, y )
#define _Out_writes_bytes_(x)				__out_bcount( x )
#define _Out_writes_bytes_opt_(x)			__out_bcount_opt( x )
#define _Out_writes_bytes_to_(x, y)			__out_bcount_part( x, y )
#define _Out_writes_bytes_to_opt_(x, y)		__out_bcount_part_opt( x, y )
#define _Out_writes_opt_(x)					__out_ecount_opt( x )
#define _Outptr_result_buffer_(x)			__deref_out_ecount( x )
#define	_Outptr_result_maybenull_ 			__deref_out_opt
...
#if ( JET_VERSION >= 0x0602 )
#define JET_paramMaxTransactionSize 			178	//	Percentage of version store that can be used by oldest transaction before JET_errOutOfVersionStore (default = 100).
...
// System parameters 184 and 185 are reserved.
...
#define JET_errSecondaryIndexCorrupted		-1414 /* Secondary index is corrupt. The database must be defragmented. If the corrupt index is over Unicode text, a likely cause a sort-order change. */
