...
// Enum and struct for CLUSCTL_RESOURCE_CSV_SW_PROVIDER_EVENT
typedef enum _CLUSTER_CSV_SWPRV_EVENT {
    SwprvSetContext,
    SwprvGetSnapshotProperties,
    SwprvSetSnapshotProperty,
    SwprvIsVolumeSupported,
    SwprvIsVolumeSnapshotted,
    SwprvBeginPrepareSnapshot,
    SwprvEndPrepareSnapshots,
    SwprvPreCommitSnapshots,
    SwprvCommitSnapshots,
    SwprvPostCommitSnapshots,
    SwprvPreFinalCommitSnapshots,
    SwprvPostFinalCommitSnapshots,
    SwprvCreateSnapshotCsv,
    SwprvDeleteSnapshotCsv,
    SwprvDeleteSnapshots,
    SwprvAbortSnapshots,
    SwprvGetAllSnapshots,
    SwprvAddDiffArea,
    SwprvChangeDiffAreaMaxSize,
    SwprvQueryVolumesSupportedForDiffAreas,
    SwprvQueryDiffAreasForVolume,
    SwprvQueryDiffAreasOnVolume,
    SwprvQueryDiffAreasForSnapshot,
    SwprvSetVolumeProtectLevel,
    SwprvGetVolumeProtectLevel,
    SwprvClearVolumeProtectFault,
    SwprvDeleteUnusedDiffAreas,
    HwprvPreCommitSnapshot,
    HwprvPostCommitSnapshot,
    PrepareForFreeze,
    SwprvMaxEventId
} CLUSTER_CSV_SWPRV_EVENT, *PCLUSTER_CSV_SWPRV_EVENT;

typedef struct _CLUS_CSV_SWPRV_INFO {
    CLUSTER_CSV_SWPRV_EVENT Event;
    union {
        struct {
        ...
        struct {
            ...
            WCHAR   ServiceHost[MAX_PATH];
            WCHAR   SnapshotCsvDevice[MAX_PATH];
            WCHAR   VolumeName[MAX_PATH];
        } CSCIn;
        struct {
            GUID    SnapshotId;
        ...
        struct {
            WCHAR   VolumeName[MAX_PATH];
            WCHAR   DiffAreaVolumeName[MAX_PATH];
            LONGLONG MaxDiffAreaSize;
        } ADAIn;
        struct {
            WCHAR   VolumeName[MAX_PATH];
            WCHAR   DiffAreaVolumeName[MAX_PATH];
...
} CLUS_CSV_SWPRV_INFO, *PCLUS_CSV_SWPRV_INFO;
...
typedef struct _CLUS_SNAPSHOT_DIFF_PROP {
    ...
    LONGLONG  MaximumDiffSpace;
    union {
        LONGLONG  AllocatedDiffSpace;
        LONGLONG  VolumeFreeSpace;
    };
    union {
        LONGLONG  UsedDiffSpace;
        LONGLONG  VolumeTotalSpace;
    };
} CLUS_SNAPSHOT_DIFF_PROP, *PCLUS_SNAPSHOT_DIFF_PROP;

typedef struct _CLUS_CSV_SWPRV_RESPONSE {
    HRESULT     Result;
    union {
        struct {
            BOOL SnapshotsPresent;
            LONG SnapshotCompatibility;
        } IVSOut;
        struct {
            GUID  SnapshotId;
            GUID  SnapshotSetId;
            LONG  SnapshotCount;
            WCHAR SnapshotDeviceName[50];
            WCHAR SnapshotPseudoVolume[64];
            WCHAR OriginalVolumeName[MAX_PATH];
            WCHAR OriginatingMachine[MAX_PATH];
            WCHAR ServiceMachine[MAX_PATH];
            WCHAR ExposedName[MAX_PATH];
            WCHAR ExposedPath[MAX_PATH];
            GUID  ProviderId;
            LONG  SnapshotAttributes;
        ...
        struct {
            BYTE  VolumeProtectLevel[24];
        } VPLOut;
        struct {
            DWORD DiffCount;
            CLUS_SNAPSHOT_DIFF_PROP DiffAreas[1];
        } DAPOut;
    };
} CLUS_CSV_SWPRV_RESPONSE, *PCLUS_CSV_SWPRV_RESPONSE;
// End of stuff for CSV SW provider
