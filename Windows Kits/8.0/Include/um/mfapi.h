...
// Note: only inbox SAR should set this attribute on MEStreamSinkFormatChanged event
// MF_EVENT_FORMAT_CHANGE_REQUEST_SOURCE_SAR {B26FBDFD-C32C-41fe-9CF0-083CD5C7F8A4}
// Type: UINT32
DEFINE_GUID(MF_EVENT_FORMAT_CHANGE_REQUEST_SOURCE_SAR,
0xb26fbdfd, 0xc32c, 0x41fe, 0x9c, 0xf0, 0x8, 0x3c, 0xd5, 0xc7, 0xf8, 0xa4);
...
STDAPI MFCopyImage(
    _Out_writes_bytes_(abs(lDestStride) *  dwLines) BYTE* pDest,
    LONG    lDestStride,
    _In_reads_bytes_(abs(lSrcStride) * dwLines) const BYTE* pSrc,
    LONG    lSrcStride,
    _Out_range_(<=, min(abs(lSrcStride), abs(lDestStride)))  DWORD dwWidthInBytes,
    DWORD   dwLines
    );
