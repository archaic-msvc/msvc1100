...
//
// These functions are exposed for MRT string caching only
//
WINBASEAPI
DWORD
WINAPI
_OpenMuiStringCache(
    _In_ PHANDLE pCacheKey,
    _In_ PWSTR CachePath,
    _In_ PCWSTR GenOrFileName,
    _In_ PCWSTR LangChecksum,
    _In_reads_(cchLangsList) PCWSTR LangsList,
    _In_ ULONG cchLangsList
    );
...
_AddMUIStringToCache(
    _In_ HANDLE hCacheKey,
    _In_ PCWSTR CacheID,
    _In_reads_(cchSize) PCWSTR Buffer,
    _In_  DWORD cchSize
    );
...
_GetMUIStringFromCache(
    _In_ HANDLE hMuiStringCache,
    _In_ PCWSTR CacheID,
    _In_ ULONG CacheFlags,
    _Out_writes_to_opt_(cchBuffer, *pcchBufferOut) PWSTR Buffer,
    _In_ DWORD cchBuffer,
    _Out_ PDWORD pcchBufferOut
    );
