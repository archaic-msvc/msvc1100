...
#define InspectableClassStatic(runtimeClassName, trustLevel) \
    public: \
        ...
        STDMETHOD(GetIids)(_Out_ ULONG *iidCount, _Outptr_result_buffer_(*iidCount) IID **iids) \
        { \
...
public:
    ...
    HRESULT GetActivationFactory(_In_opt_ HSTRING pActivatibleClassId, _Outptr_ IActivationFactory **ppIFactory, _In_opt_z_ const wchar_t* serverName = nullptr) throw()
    {
        return Details::GetActivationFactory<InProc>(this, serverName, pActivatibleClassId, ppIFactory);
    }

    HRESULT GetClassObject(REFCLSID clsid, REFIID riid, _Outptr_ void **ppv, _In_opt_z_ const wchar_t* serverName = nullptr) throw()
    {
